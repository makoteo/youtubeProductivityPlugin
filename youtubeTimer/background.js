...
var totalTime = 0; //
var interval = [0, 0, 0]; //first item is overwrite flag (0 means youtube isn't on, 1 means it was already open), second is start date, third is end date
var lockedTimeLimit = 2;

//DELETE BEFORE PUBLISH ----------------------------------------------------------------------------
//chrome.storage.local.set({"totalTime": totalTime}, function() {}); //RESET TOTAL TIME

//SET VARIABLES
chrome.storage.local.set({"difficulty": 3}, function() {}); 
chrome.storage.local.set({"timeLimit": 2}, function() {});  
...
//Log time when you click on different tab
...
chrome.tabs.onActivated.addListener( function (activeInfo) {
	chrome.tabs.getSelected(function(tab){trackTime(tab.url, new Date, tab.id);});
});
...
function setMidnightFlag(){
	...
	var millisTill = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 12, 41, 00, 0) - now;
	if (millisTill <= 0) {
	...
	setTimeout(function(){
		...
		//chrome.storage.local.set({"startTime": interval[1].toString()}, function() {});
		chrome.storage.local.get(['timeLimit'], function(result) { //Gets the current time set by content.js
			lockedTimeLimit = result.timeLimit;
